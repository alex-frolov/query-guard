...
final class DefaultQueryCollectorTest extends TestCase
{
    ...
    public function testMaxTraceQueriesIsPassedToTheTrace(): void
    {
        $collector = new DefaultQueryCollector(maxTraceQueries: 1);

        $trace = $collector->beginTrace($this->test(), TestOptions::none());
        $collector->record($this->event('SELECT 1'));
        $collector->record($this->event('SELECT 2'));

        self::assertTrue($trace->isTruncated());
        self::assertSame(2, $trace->count());
