...
final class JsonReporterTest extends TestCase
{
    ...
    /**
     * A report from a parallel run holds one worker's share of it. Which share has to be
     * on the file, or merged reports cannot be told apart afterwards.
     */
    public function testTheWorkerTokenIsOnTheReport(): void
    {
        self::assertNull($this->write(new Report())['worker']);

        $this->path = tempnam(sys_get_temp_dir(), 'qg') ?: '';

        (new JsonReporter($this->path, '/app', Severity::Warning, '7'))->report(new Report(), Mode::Report);

        $decoded = json_decode((string) file_get_contents($this->path), true);

        self::assertIsArray($decoded);
        self::assertSame('7', $decoded['worker']);
    }

    /**
     * `var/` is in `.gitignore` on most projects and missing from a fresh checkout. It
     * used to be the caller's job to create it, and forgetting cost a whole run on a real
     * project — see `directoryReady()`.
     */
    public function testAMissingDirectoryIsCreatedRatherThanReported(): void
    {
        $directory = sys_get_temp_dir().'/query-guard-report-'.getmypid();
        $path = $directory.'/nested/report.json';
        $report = new Report();

        try {
            (new JsonReporter($path))->report($report, Mode::Report);
     ...
     *
     * The directory that cannot be made is a plain file: `mkdir()` fails on it whoever
     * the process happens to be, whereas a path under `/` only fails for a user without
     * write access there — and a CI container is often root.
     */
    public function testAnUnwritableReportBecomesANotice(): void
    {
        ...
        try {
            ...
            self::assertSame(['could not write the JSON report to '.$path.'.'], $report->notices());
        } finally {
            @unlink($file);
        }
    }
