...
final class TraceTest extends TestCase
{
    ...
    public function testATraceWithoutALimitNeverTruncates(): void
    {
        $trace = $this->trace();
        $trace->record(new QueryEvent('SELECT 1'));

        self::assertFalse($trace->isTruncated());
    }

    /**
     * Past the limit, events stop being held — but `count()` keeps counting, so
     * `query-count` still reads the real total rather than the capped one.
     */
    public function testPastTheLimitEventsStopBeingHeldButAreStillCounted(): void
    {
        $trace = new Trace($this->test(), TestOptions::none(), maxTraceQueries: 2);

        $trace->record(new QueryEvent('SELECT 1'));
        $trace->record(new QueryEvent('SELECT 2'));
        $trace->record(new QueryEvent('SELECT 3'));

        self::assertTrue($trace->isTruncated());
        self::assertSame(2, $trace->maxTraceQueries());
        self::assertCount(2, $trace->events());
        self::assertSame(3, $trace->count());
    }

    public function testATraceUnderTheLimitDoesNotTruncate(): void
    {
        $trace = new Trace($this->test(), TestOptions::none(), maxTraceQueries: 5);
        $trace->record(new QueryEvent('SELECT 1'));
