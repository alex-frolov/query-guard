...
final class TestSubscribersTest extends TestCase
{
    ...
    /**
     * A truncated trace has to look different in the summary from a complete one, or
     * the rules' incomplete view goes unnoticed. See `Query\Trace::isTruncated()`.
     */
    public function testFinishedNoticesALoudlyOnATruncatedTrace(): void
    {
        $this->collector = new DefaultQueryCollector(maxTraceQueries: 1);
        (new PreparedSubscriber($this->collector, $this->adapters))->notify(Events::prepared());
        $this->collector->record($this->event('SELECT 1'));
        $this->collector->record($this->event('SELECT 2'));
        ...
        self::assertCount(1, $this->report->notices());
        self::assertStringContainsString('trace truncated at 1 queries', $this->report->notices()[0]);
    }

    public function testFinishedSaysNothingWhenTheTraceIsNotTruncated(): void
    {
        $this->openTrace();
