...
final class EloquentAdapterTest extends TestCase
{
    ...
    /**
     * Laravel throws the previous test's application away, and the `DB` facade keeps
     * handing back the `DatabaseManager` that belonged to it. Asking that manager to
     * `listen()` reaches `__call`, which resolves the default connection out of a
     * container `Application::flush()` has emptied, and dies. Uncaught, that is one
     * "Exception in third-party event subscriber" warning per test.
     */
    public function testAListenTargetThatThrowsIsNotAnAttachment(): void
    {
        $target = new class {
            public function listen(\Closure $listener): void
            {
                throw new \RuntimeException('Target class [config] does not exist.');
            }
        };

        self::assertFalse(EloquentAdapter::attach($target, $this->collector));

        $adapter = new EloquentAdapter($target);
        $adapter->install($this->collector);
