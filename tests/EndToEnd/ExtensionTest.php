...
<phpunit bootstrap="%s" cacheDirectory="%s" colors="false">
    ...
    /**
     * The callsite is the shared method every caller goes through; the chain is the
     * caller. The second cannot be derived from the first, and on Eloquent the callsite
     * alone is barely usable at all.
     */
    public function testAFindingCarriesTheCallersAboveItsCallsite(): void
    {
        [$output] = $this->runFixture('phpunit-chain.xml');

        self::assertStringContainsString('tests/EndToEnd/Fixture/Chain/OrderRepository.php:', $output, $output);
        self::assertStringContainsString('from tests/EndToEnd/Fixture/Chain/ChainTest.php:', $output, $output);
        self::assertStringContainsString('OrderRepository::find', $output, $output);
    }

    public function testTheChainCanBeTurnedOff(): void
    {
        // written next to the fixture it is a copy of: paths inside a PHPUnit
        // configuration resolve against the file's own directory
        $configuration = __DIR__.'/Fixture/phpunit-chain-off-'.getmypid().'.xml';

        file_put_contents($configuration, str_replace(
            '<parameter name="mode" value="report"/>',
            '<parameter name="mode" value="report"/><parameter name="call-chain-depth" value="0"/>',
            (string) file_get_contents(__DIR__.'/Fixture/phpunit-chain.xml'),
        ));
        ...
        try {
            ...
            self::assertStringContainsString('tests/EndToEnd/Fixture/Chain/OrderRepository.php:', $output, $output);
            self::assertStringNotContainsString('from tests/EndToEnd/Fixture/Chain/ChainTest.php:', $output, $output);
        } finally {
            @unlink($configuration);
        }
    }
    ...
    /**
     * `--no-output` with nowhere else to write is the one case that stays silent: the
     * user asked for silence and no file was configured, so collecting would only cost.
     */
    public function testNoOutputWithNowhereToWriteStaysOutOfTheWay(): void
    {
        [$output, $exitCode] = $this->runFixture('phpunit.xml', [], '--no-output');

        self::assertStringNotContainsString('query-guard', $output, $output);
        self::assertSame(0, $exitCode, $output);
    }

    /**
     * A silenced console is not a reason to skip a configured file. This is what a
     * `report-json` in CI relies on, and what a Pest worker used to lose entirely.
     */
    public function testNoOutputStillFillsTheJsonReport(): void
    {
        $report = sys_get_temp_dir().'/query-guard-no-output-'.getmypid().'.json';
        $configuration = $this->temporaryTier1Configuration('', sprintf('<parameter name="report-json" value="%s"/>', $report));

        try {
            [$output] = $this->runConfiguration($configuration, [], '--no-output');
            ...
            self::assertIsArray($decoded);
            self::assertIsArray($decoded['findings'] ?? null);
            self::assertNotSame([], $decoded['findings']);
        } finally {
            @unlink($report);
            @unlink($configuration);
        }
    ...
    /**
     * Pest appends `--no-output` because Collision prints instead of PHPUnit, not
     * because anyone asked for quiet — and it announces itself through
     * `COLLISION_PRINTER` before PHPUnit ever starts. The summary has to survive that.
     */
    public function testAReplacedPrinterIsNotARequestForSilence(): void
    {
        [$output] = $this->runFixture('phpunit.xml', ['COLLISION_PRINTER' => 'DefaultPrinter'], '--no-output');

        self::assertStringContainsString('tests traced: 6, queries: 28 (in setUp: 3)', $output, $output);
    }

    /**
     * Under ParaTest every worker is a process of its own writing the one path from
     * `report-json`, and the last to finish wins — 79% of a run went missing that way on
     * a real Laravel suite. `%token%` gives each of them a file. The directory does not
     * exist here either: a fresh checkout has no `var/`.
     */
    public function testAWorkerWritesTheReportUnderItsOwnToken(): void
    {
        $directory = sys_get_temp_dir().'/query-guard-worker-'.getmypid();
        $configuration = $this->temporaryTier1Configuration('', sprintf('<parameter name="report-json" value="%s/report-%%token%%.json"/>', $directory));

        try {
            $this->runConfiguration($configuration, ['TEST_TOKEN' => '7']);

            self::assertFileExists($directory.'/report-7.json');

            $decoded = json_decode((string) file_get_contents($directory.'/report-7.json'), true);

            self::assertIsArray($decoded);
            self::assertSame('7', $decoded['worker'] ?? null);
        } finally {
            @unlink($directory.'/report-7.json');
            @rmdir($directory);
            @unlink($configuration);
        }
    }
    ...
    /**
     * The same run without the placeholder: the file is still written, because a partial
     * report is better than none — but the summary has to say that this is one worker's
     * share overwriting everybody else's.
     */
    public function testAWorkerSharingOneReportPathIsToldSo(): void
    {
        $report = sys_get_temp_dir().'/query-guard-shared-'.getmypid().'.json';
        $configuration = $this->temporaryTier1Configuration('', sprintf('<parameter name="report-json" value="%s"/>', $report));

        try {
            [$output] = $this->runConfiguration($configuration, ['TEST_TOKEN' => '5']);

            self::assertStringContainsString('this is ParaTest worker 5', $output, $output);
            self::assertStringContainsString('%token%', $output, $output);
            self::assertFileExists($report);
        } finally {
            @unlink($report);
            @unlink($configuration);
    ...
    /**
     * A baseline cannot be split by worker the way a report can: it is one committed
     * file. Twelve workers writing it leaves whichever share finished last, and the run
     * after that fails on findings nobody added.
     */
    public function testABaselineIsNotRegeneratedInsideAWorker(): void
    {
        $baseline = sys_get_temp_dir().'/query-guard-worker-baseline-'.getmypid().'.json';
        $configuration = $this->temporaryTier1Configuration($baseline);

        try {
            [$output] = $this->runConfiguration($configuration, ['QUERY_GUARD_GENERATE_BASELINE' => '1', 'TEST_TOKEN' => '3']);

            self::assertStringContainsString('this is ParaTest worker 3', $output, $output);
            self::assertStringContainsString('regenerate the baseline in a sequential run', $output, $output);
     ...
     */
    private function runFixture(string $configuration, array $environment = [], string $arguments = ''): array
    {
        return $this->runConfiguration(__DIR__.'/Fixture/'.$configuration, $environment, $arguments);
    }
     ...
     * @return array{0: string, 1: int}
     */
    private function runConfiguration(string $path, array $environment = [], string $arguments = ''): array
    {
        $prefix = '';
