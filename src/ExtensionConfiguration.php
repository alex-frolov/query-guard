 ...
 *
 * `max-trace-queries` is off by default for the same reason as `tier2` and `max-queries`:
...
final readonly class ExtensionConfiguration
{
     ...
     * @param list<string> $skipFunctions extra `Class::method` fragments a frame is never
     *                                    resolved to — plumbing that lives in the
     *                                    project's own files
     * @param list<string> $fixturePaths  path fragments whose queries count as building
     *                                    the stand rather than exercising the application
    ...
    public function __construct(
        ...
        public ?int $maxTraceQueries = null,
        ...
        public array $skipFunctions = [],
        public array $fixturePaths = [],
        public int $callChainDepth = CallsiteResolver::DEFAULT_CHAIN_DEPTH,
    ...
    public static function fromParameters(ParameterCollection $parameters): self
    {
        ...
        return new self(
            ...
            skipFunctions: self::list($parameters, 'skip-functions'),
            fixturePaths: self::list($parameters, 'fixture-paths'),
            callChainDepth: self::positiveInt($parameters, 'call-chain-depth', CallsiteResolver::DEFAULT_CHAIN_DEPTH, 0, $warnings) ?? CallsiteResolver::DEFAULT_CHAIN_DEPTH,
