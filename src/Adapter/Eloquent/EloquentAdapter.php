...
final class EloquentAdapter implements OrmAdapter
{
    ...
    public static function attach(object $target, ?QueryCollector $collector = null): bool
    {
        ...
        if ($target instanceof Dispatcher) {
            ...
            //
            // The call can throw, and on Laravel it routinely does. Between tests the
            // `DB` facade still hands back the previous test's `DatabaseManager`, whose
            // container `Application::flush()` has already emptied; `listen()` is not a
            // method of the manager, so `__call` resolves the default connection and
            // dies on `Target class [config] does not exist`. Thrown out of a PHPUnit
            // subscriber that becomes one "Exception in third-party event subscriber"
            // warning per test — 1611 of them on a live suite. Failing to attach here
            // costs nothing: the application is rebuilt moments later and the service
            // provider, or `Test\Prepared`, subscribes to a live dispatcher.
            try {
