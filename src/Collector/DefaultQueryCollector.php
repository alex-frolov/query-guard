...
final class DefaultQueryCollector implements QueryCollector
{
    ...
    public function __construct(
        ...
        private readonly ?FixtureFilter $fixtures = null,
    ) {
    }
    ...
    public function record(QueryEvent $event): void
    {
        ...
        switch ($this->phase) {
            case Phase::Test:
                // a query the test made from a path the project declared to be
                // preparation goes where `setUp()`'s queries go, not to the rules
                if (null !== $this->trace && null !== $this->fixtures && $this->fixtures->isFixture($event)) {
                    $this->trace->recordFixture($event);
    ...
    {
        $this->trace = new Trace($test, $options, $this->fixtureEvents, $this->maxTraceQueries);
        $this->fixtureEvents = [];
