        ...
        if (!$report->hasFindings()) {
            ...
            foreach (self::sorted($report->findings()) as $finding) {
                ...
                if (null !== $finding->callsite) {
                    ...
                    // who called it, on findings worth chasing. `select-star` is `info`
                    // and fires on every Eloquent query there is: three more lines under
                    // each of those would bury the rest of the summary
                    if (Severity::Info !== $finding->severity) {
                        foreach ($finding->callsite->chain as $frame) {
                            $lines[] = sprintf('      from %s', $this->shorten($frame));
                        }
                    }
                }
            }
